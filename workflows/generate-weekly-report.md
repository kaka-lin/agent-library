...
### 主輔來源原則 (Primary / Auxiliary Source Policy)

掃描多個根目錄時，依「主輔關係」決定納入範圍，而非一視同仁全收：

- **主來源 (Primary)**：`/path/to/folder1` 為核心工作來源，**所有**有變動的 repo 一律全數納入。
- **輔助來源 (Auxiliary)**：`/path/to/folder2` 為輔助來源，**僅納入與當期主來源工作直接相關**的 repo（例如 `repo-a`、`repo-b` 支撐主產品 `main-product` 的核心功能），其餘與主來源無關聯的 repo（個人筆記、實驗專案等）一律略過。
- **判斷依據**：以「該 repo 的變動是否服務於同一個主產品/目標」為準；相關的輔助 repo 應盡量**掛在對應的主題之下**（如 `repo-a` 掛在其支撐的功能主題），而非自成獨立分類。
...
- **彙整技巧**：不應只是條列 commit，應將多個相關的 commit 彙總成一個具有「進度感」的短句描述。同性質的多個 commit（如「去重、防重複點擊、錯誤歸因、DOM 驗證、反機器人節奏」這類同屬「可靠性／擬人化」的工作）應**大膽合併成單一條目**，避免逐筆條列造成主題稀釋；每個主題下控制在精煉的少數幾條。
- **動態調整**：AI 應根據當週實際變動的屬性，靈活選擇最貼切的 3-4 個分類。
- **主輔歸併**：依「主輔來源原則」，主來源全收；輔助來源僅保留與當期主來源工作相關者，並優先掛在對應的主題之下（repo 條目可加註角色，如 `repo-a`（執行環境）、`repo-b`（功能模組）），讓主輔關係在報告結構中一目了然。
- **保留 repo 原名**：報告中一律直接使用 repo 名稱（如 `hermes-agent`），**不要**自行翻譯或替換為產品名／專案代稱；使用者會在檢視時自行替換對應的對外名稱。

### 5. 產出報告檔案

- **互動步驟**：詢問使用者報告存檔路徑（若本地無指定預設目錄）。
- **中文先審**：寫入檔案前，**先以中文版內文呈現給使用者檢視**（標題仍維持中英雙語），確認結構與內容無誤後，再寫入檔案。
- **產出格式與規範**：
  - 產出檔案名稱：`YYYYMMDD_weekly_report.md`。
  - **內文語言**：最終寫入檔案採「中英雙語標題 + 英文內文」（沿用既有報告慣例）；中文僅用於寫檔前的審閱階段。
